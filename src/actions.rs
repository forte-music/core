...
pub fn add_song_stats(song_stats: &SongUserStats, db: &Connection) -> RedisResult<()> {
    db.hset_multiple::<_, _, _, ()>(
        SongUserStats::key(&song_stats.id),
        &[
            ("id", &song_stats.id),
...
pub fn add_playlist_items_to_playlist(
    playlist_id: &str,
...
pub fn add_playlist_item(playlist_item: &PlaylistItem, db: &Connection) -> RedisResult<()> {
    db.hset_multiple::<_, _, _, ()>(
        PlaylistItem::key(&playlist_item.id),
        &[
            ("id", &playlist_item.id),
